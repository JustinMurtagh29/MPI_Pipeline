...
for i = idxTrain
    ...
    negSegIds = negSegIds(randperm(length(negSegIds), ceil(0.1*length(negSegIds)))); % choose x% of ids
...
skels.write(fullfile(param.saveFolder,'typeEM','spine', featureSetName,sprintf('%s_fps-%s-BoxDenselyUndersampling.nml',timeStamp, className)));

% Look at true positives
curCount = 100;
className = 'spinehead';
skels = Debug.inspectTPs(param, curCount, className, curGtTest);
skels.write(fullfile(param.saveFolder,'typeEM','spine', featureSetName,sprintf('%s_tps-%s-BoxDenselyUndersampling.nml',timeStamp, className)));
