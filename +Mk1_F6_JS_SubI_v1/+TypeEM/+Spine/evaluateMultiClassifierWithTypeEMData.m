...
% Multi classifier for 4 classes
% training set
% Positive and Negative labels:
%   N-1 boxes dense-spine-head labels and
%   13 typeEM boxes with Axon/Dend/Glia labels
...
methodUsed = 'AdaBoostM2'; %'AdaBoostM1'; % 'LogitBoost';
numTrees = 1500;
numNmlsToUse = 22;
addNoise = true;
noiseDev = 0.01;
...
%for idxTest = gtFiles
    ...
    if addNoise
        experimentName = sprintf('multiClass_%s_%d_nmls_%d_over_%.2f_under_%.2f_cost_100_addNoise_%.3f_testset_%d_GT_%s_features_%s_evalForAgglo_%d', ...
                methodUsed, numTrees, numNmlsToUse, factorPos, factorNeg, noiseDev, idxTest, gtVersion, featureSetName, evalForAgglos);
    else
        experimentName = sprintf('multiClass_%s_%d_nmls_%d_over_%.2f_under_%.2f_cost_100_noNoise_testset_%d_GT_%s_features_%s_evalForAgglo_%d', ...
            methodUsed, numTrees, numNmlsToUse, factorPos, factorNeg, idxTest, gtVersion, featureSetName, evalForAgglos);
    end

    % load train set for Spineheads
    curNodes = table();
    gtSH = struct;
    gtSH.segId = [];
    gtSH.label = [];
    ...
    gtSH.class = categorical({'spinehead', 'glia', 'axon', 'dendrite'});
    gtSH.segId = posSegIds;
    gtSH.label = [ones(numel(gtSH.segId),1), -1*ones(numel(gtSH.segId),3)]; %[+1,-1,-1,-1]
    ...
    gt.class = categorical({'spinehead', 'glia', 'axon', 'dendrite'});
    
    gt.label = zeros(numel(gt.segId), numel(gt.class));
    gt.label = cat(1, [-1*ones(numel(gtType.segId),1), gtType.label],... % [-,1 x,x,x]
                      gtSH.label); % [+1,-1,-1,-1] 
    ...
    gtTest = struct;
    gtTest.class = categorical({'spinehead', 'glia', 'axon', 'dendrite'});
    gtTest.segId = cat(1, double(posSegIds), double(negSegIds));
    gtTest.label = -1*ones(numel(gtTest.segId), numel(gtTest.class));
    
    % label SHs as positive
    ...
    for curTrainSize = trainSizes
        ...
        curClassifier.classifiers  = {buildMultiClass(gtTrain, methodUsed, numTrees)};
        ...
        try
            ...
            warning('Classifier not evaluated for remaining classes')
        end
...
function classifier = buildMultiClass(gt, methodUsed, numTrees)
% build multi class classifier
    % extract training data
    trainFeat = gt.featMat;
    trainClass = zeros(size(gt.label,1),1);
    for classIdx = 1:4
        trainClass(gt.label(:,classIdx) > 0) = classIdx-1;
    end
