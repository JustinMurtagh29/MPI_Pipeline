...
numTreesList = [2000, 1500, 1000, 500, 50];
...
for i = idxTrain
    ...
    posSegIds = repmat(posSegIds,50,1);
...
skels.write(fullfile(param.saveFolder,'typeEM','spine', featureSetName,sprintf('%s_fps-%s-BoxOversamplingWTrees.nml',timeStamp, className)));

% Look at true positives
curCount = 100;
className = 'spinehead';
skels = Debug.inspectTPs(param, curCount, className, curGtTest);
skels.write(fullfile(param.saveFolder,'typeEM','spine', featureSetName,sprintf('%s_tps-%s-BoxOversamplingWTrees.nml',timeStamp, className)));
