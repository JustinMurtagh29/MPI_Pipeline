...
oldMag4 = true;
outputFolder = '/tmpscratch/sahilloo/data/Mk1_F6_JS_SubI_v1/nucleiRun/';
if ~exist(outputFolder)
    mkdir(outputFolder);
end
dataset.root = '/tmpscratch/jast/wKcubes/Mk1_F6_JS_SubI_v1/color/1/';
dataset.backend = 'wkwrap';
dataset.blocksize = 32;
mag1bbox_wk = [0, 0, 0, 24064, 24064, 5120];
mag1bbox = Util.convertWebknossosToMatlabBbox(mag1bbox_wk);

%% set some folders
datasetMag4 = dataset;
if oldMag4
    datasetMag4.root = strrep(dataset.root,'/1/','/4-4-4/');
else
    datasetMag4.root = strrep(dataset.root,'/1/','/4-4-2/');
end
datasetMag4Heur = datasetMag4;
datasetMag4Heur.root = fullfile(outputFolder,'/heur/');
...
job = Cluster.startJob( ...
    @detectNucleiInCube, offsets1,'sharedInputs',{datasetMag4, datasetMag4Heur,cubesize,delObjSmallerThan,true,minmaxArea,oldMag4}, ...
...
job = Cluster.startJob( ...
    @detectNucleiInCube, offsets2,'sharedInputs',{datasetMag4, datasetMag4Heur,cubesize,delObjSmallerThan,true,minmaxArea,oldMag4}, ...
    'sharedInputsLocation',[1,2,3,4,6,7,8],'cluster', {'memory', 12,'time', '30:00:00','taskConcurrency',20,'scheduler','slurm'}, ...
    'name', 'nucleiDetection');
Cluster.waitForJob(job);
