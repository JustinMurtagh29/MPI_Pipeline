function visualizeTasksNewNew(seg, v, parameter, id, i, colors)
...
for vp =1:length(v)
    ...
    p = v(vp).missions(i).start.CoM .* parameter.settings.scale;
    h1 = plot3(p(1), p(2), p(3), 'xr', 'MarkerSize', 10, 'LineWidth', 3);
    hold on;
    % Plot CoM end object
    p = v(vp).missions(i).end.CoM .* parameter.settings.scale;
    h2 = plot3(p(1), p(2), p(3), 'xg', 'MarkerSize', 10, 'LineWidth', 3);
    % Plot error center
    p = v(vp).missions(i).errorCenter .* parameter.settings.scale;
    ...
    p = v(vp).missions(i).errorCenter .* parameter.settings.scale + v(vp).missions(i).start.firstFrame*v(vp).missions(i).start.direction;  
    ...
    p = v(vp).missions(i).errorCenter .* parameter.settings.scale + v(vp).missions(i).possibleEnds(1).firstFrame*v(vp).missions(i).start.direction; 
...
%         bbox = parameter.bboxSmall' .* repmat(parameter.settings.scale, 2, 1);
...
%         plot3([bbox(2,1) bbox(2,1)], [bbox(2,2) bbox(2,2)], [bbox(1,3) bbox(2,3)], ':m', 'LineWidth', 3);
    ...
    issf.vertices = bsxfun(@times, bsxfun(@plus, issf.vertices, parameter.bboxBig(:,1)'), parameter.settings.scale);
    h8 = patch(issf);
...
%     end
    ...
    bbox = [(v(vp).missions(i).errorCenter - parameter.kdbBorder).* parameter.settings.scale; ...
        (v(vp).missions(i).errorCenter + parameter.kdbBorder).* parameter.settings.scale];
    ...
    for x=1:2
        for y=1:2
            for z=1:2
                edgePos{x, y, z} = v(vp).missions(i).errorCenter .* parameter.settings.scale + ...
    ...
    view(3);
    title(['Mission ID: ' num2str(v(vp).missions(i).missionId) ', Game: ' v(vp).name ', Error center: ' num2str(v(vp).missions(i).errorCenter)]);
    grid on;
    camlight('headlight');
    lighting phong;
    ...
    axis equal;
    saveas(gcf, ['C:\Users\mberning\Desktop\problemInspector\' id v(vp).name num2str(v(vp).missions(i).missionId, '%.7i') 'isosurface.fig']);
    set(gcf,'PaperSize',fliplr(get(gcf,'PaperSize')));
    print(gcf, ['C:\Users\mberning\Desktop\problemInspector\' id v(vp).name num2str(v(vp).missions(i).missionId, '%.7i') 'isosurface.pdf'], '-dpdf', '-r300');
    close all;
    ...
    subplot(1,2,1);
    hold on;
    plot([0 0], [startFF(i) startLF(i)], 'Color', colors(1,:), 'LineWidth', 2);
    labels{1} = num2str(v(vp).missions(i).start.id);
    for j=1:length(v(vp).missions(i).possibleEnds)
        if ~isempty(endFF{i,j})
            plot([j j], [endFF{i,j} endLF{i,j}], 'Color', colors(1+j,:), 'LineWidth', 2);
            labels{1+j} = num2str(v(vp).missions(i).possibleEnds(j).id);
        end
        plot(j, v(vp).missions(i).possibleEnds(j).touchFrame, 'x', 'MarkerSize', 10, 'LineWidth', 2, 'Color', colors(1+j,:));
    end
    title(['Task ' num2str(v(1).missions(1).missionId) ': original, rendering length']);
    xlabel('Object IDs');
    ...
    colormap(colors(2:1+length(v(vp).missions(i).possibleEnds),:));
    set(gca, 'Clim', [1 length(v(vp).missions(i).possibleEnds)]);
    labels = labels(2:end);
    title(['Task ' num2str(v(1).missions(1).missionId) ': original, probabilities']);
    set(gca, 'Xtick', 1:length(v(vp).missions(i).possibleEnds));
    set(gca, 'XTickLabel', labels);
    xlim([0.5 length(v(vp).missions(i).possibleEnds)+0.5]);
    xlabel('Object IDs');
    ylabel('probability [%]');    
    saveas(gcf, ['C:\Users\mberning\Desktop\problemInspector\' id v(vp).name num2str(v(vp).missions(i).missionId, '%.7i') 'statistics.pdf']);
    close all;
