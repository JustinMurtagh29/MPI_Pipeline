function [feat, border, numFeatures] = maximumFilter( raw, siz )
%MAXIMUMFILTER Calculates a multidimensional minimum filter.
...
if length(siz) == 1
    siz = repmat(siz, [1, ndims(raw)]);
end
feat = imdilate(raw,ones(siz));
end
