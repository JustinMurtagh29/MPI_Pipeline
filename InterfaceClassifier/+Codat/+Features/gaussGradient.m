function [grad, border, numFeatures] = gaussGradient(raw, sigma, truncate)
...
%                 deviations.
%                 (Default: 3)
% OUTPUT grad: Cell array of length ndims(raw). Each cell contains the
%              gauss derivative wrt to the cell coordinate.
...
numFeatures = length(sigma);
if isempty(raw)
    grad = [];
    return;
end

grad = cell(ndims(raw),1);
for dim = 1:ndims(raw)
    order = zeros(ndims(raw),1);
    order(dim) = 1;
    grad{dim} = Codat.Features.gaussianFilter(raw, sigma, truncate, order);
end
