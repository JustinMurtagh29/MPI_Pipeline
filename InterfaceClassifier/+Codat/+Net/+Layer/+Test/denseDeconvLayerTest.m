classdef denseDeconvLayerTest < matlab.unittest.TestCase
    ...
    methods (Test)
        function testConvAlgs(testCase)
            %without sparsity
            layer = Codat.Net.Layer.denseDeconvolutionalLayer([3, 3, 3], [1, 1, 1], 2, 3, [1, 1, 1]);
            ...
            %with sparsity
            layer = Codat.Net.Layer.denseDeconvolutionalLayer([3, 3, 3], [1, 1, 1], 2, 3,[2, 2, 2]);
            layer = layer.initializeWeights('fan-in',0,'tanh');
            layer = layer.castParams(@double);
            layer2 = layer.setConvAlg('fft2');
            layer3 = layer.setConvAlg('convn');
