classdef denseDeconvolutionalLayer < Codat.Net.Layer.baseLayer
    ...
    %                  less memory.
    %          'convn': Convolution via MATLABs convn function.
    ...
    % wMask: Mask for sparsity used in backprop in some modes.
    %
    ...
    methods
        function obj = denseDeconvolutionalLayer(filterSize, stride, featureMapsIn, featureMapsOut, sparsityFactor)
        ...
        function [layer, Y] = forward(layer, X)
            X = padarray(X, layer.sparseFilterSize - 1, layer.padVal);
            sX = size(X);
            sX(end+1:4) = 1;
            ...
            switch layer.convAlg
                case 'fft' %semi vectorized and adapted for strided convolutions
                    ...
                    %fft2 also making use of sparse filters in fourier space
                    %commented out because I did not have time to test and fully
                    %implement it yet
                    %                     case 'fft2'
                    %                         w = layer.W;
                    %                         fftSiz = size(X) + layer.filterSize - 1;
                    %                         fftSiz(end + 1:4) = 1;
                    %                         X = fftn(X,fftSiz);
                    %                         for fm = 1:layer.featureMapsOut
                    %                             wFM = fftn(w(:,:,:,:,fm),fftSiz);
                    %                             Y(:,:,:,fm) = real(ifftn(X.*wFM));
                    %                         end
                    %                     case 'convn'
                    %                         for fm = 1:layer.featureMapsOut
                    %                             Y(:,:,:,fm) = convn(X,layer.W(:,:,:,:,fm),'full') ...
                    %                                 + layer.b(fm);
                    %                         end
        ...
        function [DEDX, DEDW] = backward(layer, X, DEDY)
            ...
            fS = layer.sparseFilterSize;
            DEDX = DEDX(fS(1):end - fS(1) + 1, ...
                fS(2):end - fS(2) + 1, ...
                fS(3):end - fS(3) + 1,:);
            DEDW = [gW(:);gb(:)];
        end
        ...
        function layer = initializeWeights(layer, method, varargin)
            ...
            switch method
                case 'fan-in'
                    ...
                    layer.W = tmp;
                case 'randn'
                    layer.W = varargin{1}.*randn([layer.filterSize, layer.featureMapsIn, layer.featureMapsOut],'single');
                otherwise
                    error('Initialization method ''%s'' not implemented.',method);
            end
            layer = layer.setConvAlg(currConvAlg);
        end
