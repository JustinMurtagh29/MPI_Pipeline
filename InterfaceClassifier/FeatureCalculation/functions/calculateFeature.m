function X = calculateFeature( name, parameters, raw, interfaceSurfaceList, subsegmentsList, featureIndices, quantiles, sumStats, border )
...
switch name
    ...
    case 'MMV_load' %load precalculated MMV predictions
        m = load(parameters{1},'pred');
        imfeat = m.pred;
        X = [];
        for i = 1:size(imfeat,4)
            X = cat(2,X,poolFromFilterResponse(imfeat,interfaceSurfaceList,subsegmentsList,featureIndices{1},quantiles,sumStats,border));
        end

    case 'GaussianSmoothed'
        imfeat = Codat.Features.gaussianFilter(raw,parameters{:});
        X = poolFromFilterResponse(imfeat,interfaceSurfaceList,subsegmentsList,featureIndices{1},quantiles,sumStats,border);

    case 'LoG'
        imfeat = Codat.Features.laplacianOfGaussian(raw,parameters{:});
        X = poolFromFilterResponse(imfeat,interfaceSurfaceList,subsegmentsList,featureIndices{1},quantiles,sumStats,border);

    case 'DoG'
        imfeat = Codat.Features.differenceOfGaussians(raw,parameters{:});
        X = poolFromFilterResponse(imfeat,interfaceSurfaceList,subsegmentsList,featureIndices{1},quantiles,sumStats,border);

    case 'GaussGrad'
        imfeat = Codat.Features.gaussGradientMagnitude(raw,parameters{:});
        X = poolFromFilterResponse(imfeat,interfaceSurfaceList,subsegmentsList,featureIndices{1},quantiles,sumStats,border);

    case 'StructureTensor'
        imfeat = Codat.Features.eigenvaluesOfStructureTensor(raw,parameters{:});
    ...
    case 'Hessian'
        imfeat = Codat.Features.eigenvaluesOfHessian(raw,parameters{:});
        X1 = poolFromFilterResponse(imfeat{1},interfaceSurfaceList,subsegmentsList,featureIndices{1},quantiles,sumStats,border);
        X2 = poolFromFilterResponse(imfeat{2},interfaceSurfaceList,subsegmentsList,featureIndices{2},quantiles,sumStats,border);
        X3 = poolFromFilterResponse(imfeat{3},interfaceSurfaceList,subsegmentsList,featureIndices{3},quantiles,sumStats,border);
        X = cat(2,X1,X2,X3);
    ...
    case 'Entropy'
        ...
        imfeat = single(entropyfilt(uint8(rawSmoothed),ones(nhood)));
        X = poolFromFilterResponse(imfeat,interfaceSurfaceList,subsegmentsList,featureIndices{1},quantiles,sumStats,border);
    ...
    case 'SphereAverage'
        imfeat = Codat.Features.localAverage(raw,parameters{:});
        X = poolFromFilterResponse(imfeat,interfaceSurfaceList,subsegmentsList,featureIndices{1},quantiles,sumStats,border);
