function X = calculateFeatures( raw, interfaces, featureMap )
%CALCULATEFEATURES Calculate Features on output of interface calculation.
...
for nFeature = 1:length(featureMap.type)
    if any(subsref(cell2mat(featureMap.featureIndices{nFeature}),struct('type','()','subs',{{':'}})))
        X(:,iter:iter + featureMap.numSelectedFeatures(nFeature) - 1) = calculateFeature(featureMap.names{nFeature},featureMap.parameters{nFeature},raw,interfaceSurfaceList,subsegmentsList,featureMap.featureIndices{nFeature},featureMap.quantiles,featureMap.summaryStatistics,featureMap.border);
    end
