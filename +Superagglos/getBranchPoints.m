function nBPs = getBranchPoints(p,superagglos,method,show)
...
for s = 1:numel(superagglos)
    ...
    switch method
        case 'sphere'
            ...
            for n = 1:size(aggloWOsoma.nodes,1)
                % constrain superagglos(s) to everything in the outerBbox around the current
                ...
                thisAgglo = thisAgglo(arrayfun(@(x) any(ismember(x.nodes(:,1:3),aggloWOsoma.nodes(n,1:3),'rows')),thisAgglo)); % get the superagglos(s) which belongs to the current node
                ...
                if sum(Superagglos.calculateTotalPathLength(splitAgglo) > 500) > 2
                    BPs = cat(1,BPs,n);
                end
            end
        ...
        case 'thin'
            ...
            for a = 1:numel(aggloWOsoma)
                ...
                BPs = BPcands(arrayfun(@(x) sum(Superagglos.calculateTotalPathLength(Superagglos.removeNodesFromAgglo(aggloWOsoma(a),x)) > 500)> 2 ,BPcands));
    ...
    if show
        figure;hold all
        skel = Superagglos.toSkel(superagglos(s));
