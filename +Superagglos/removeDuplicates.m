...
[nodesToKeep, edgesToKeep] = arrayfun(@(x) deal(superAgglos(x).nodes(keepIndices{x},:),connectEM.changem(superAgglos.edges,uIndices,1:size(superAgglos(x).nodes,1))),1:numel(superAgglos),'uni',0);
