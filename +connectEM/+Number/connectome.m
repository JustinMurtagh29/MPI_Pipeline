...
%% Shaft synapses
curIsAdShaft = synT.targetClass == 'ApicalDendrite' & not(synT.isSpine);
curIsOdShaft = synT.targetClass == 'OtherDendrite' & not(synT.isSpine);

curIsExc = ismember( ...
    conn.axonMeta.axonClass(synT.preAggloId), ...
    {'Corticocortical', 'Thalamocortical'});
curIsInh = ismember( ...
    conn.axonMeta.axonClass(synT.preAggloId), ...
    {'Inhibitory'});

numClassifiedSpinyDendShaftSynapses = ...
    sum((curIsAdShaft | curIsOdShaft) & (curIsExc | curIsInh)) %#ok
numClassifiedSpinyDendShaftSynapsesFromExcitatoryAxons = ...
    sum((curIsAdShaft | curIsOdShaft) & curIsExc) ...
  / numClassifiedSpinyDendShaftSynapses %#ok

numClassifiedAdShaftSynapses = ...
    sum(curIsAdShaft & (curIsExc | curIsInh)) %#ok
fractionOfClassifiedAdShaftSynapsesFromExcitatoryAxons = ...
    sum(curIsAdShaft & curIsExc) / numClassifiedAdShaftSynapses %#ok

numClassifiedOdShaftSynapses = ...
    sum(curIsOdShaft & (curIsExc | curIsInh)) %#ok
fractionOfClassifiedOdShaftSynapsesFromExcitatoryAxons = ...
    sum(curIsOdShaft & curIsExc) / numClassifiedOdShaftSynapses %#ok
