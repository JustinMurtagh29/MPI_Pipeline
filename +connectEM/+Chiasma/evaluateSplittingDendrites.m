...
clear;
outputDir = '/tmpscratch/scchr/L4/chiasmata/dendrites/round4/chiasmataSplitting/evaluation/';
rootDir = '/gaba/u/mberning/results/pipeline/20170217_ROI';
load(fullfile(rootDir, 'allParameter.mat'), 'p');
param = p;

oldAgglos = load(fullfile( ...
    p.saveFolder, 'aggloState', 'dendrites_flight_02.mat'));
agglos = oldAgglos.dendrites(oldAgglos.indBigDends);

curDir = '/tmpscratch/scchr/L4/chiasmata/dendrites/round4/chiasmataSplitting';
curData = load(fullfile(curDir, '20171115T135555_splitData.mat'));
...
for curIdx = 1:size(randQueries, 1)
    ...
    curAxon = agglos(curQuery.axonId);
...
save(strcat(outputDir,GTtable.mat), 'randQueries')
%%
...
gtFile = '/tmpscratch/scchr/L4/chiasmata/dendrites/round4/chiasmataSplitting/evaluation/GTtable3.mat';
...
for curIdx = 1:size(gtQueries, 1)
   ...
   [~, curSummary] = ...
        connectEM.splitChiasmataMulti( ...
            p, chiParam, curAxon, curQueries, 'dryRun', true);
...
for curAxonIdx = 1:numel(uniAxonIds)
    curAxon = agglos(uniAxonIds(curAxonIdx));
    curAxon.nodesScaled = bsxfun(@times, ...
        curAxon.nodes(:, 1:3), p.raw.voxelSize);
    curQueries = queries( ...
