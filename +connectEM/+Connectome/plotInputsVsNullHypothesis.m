...
%% reassemble whole cells
cellComps = conn.denMeta(conn.denMeta.cellId > 0, :);

% Only consider dendrites and soma for now
cellComps(cellComps.targetClass == 'AxonInitialSegment', :) = [];

[cellIds, cellMeta, cellComps.cellId] = unique(cellComps.cellId);

cellMeta = cellComps(cellMeta, :);
cellMeta.id = numel(conn.dendrites) + (1:height(cellMeta))';
cellMeta.parentId(:) = 0;
cellMeta.targetClass(:) = {'FullInput'};
cellMeta.synCount = accumarray( ...
    cellComps.cellId, cellComps.synCount);
cellMeta.spineSynCount = accumarray( ...
    cellComps.cellId, cellComps.spineSynCount);

cellClassConn = accumarray( ...
    cellComps.cellId, cellComps.id, [], @(ids) {ids});
cellClassConn = cell2mat(cellfun( ...
    @(ids) sum(classConnectome(ids, :), 1), ...
    cellClassConn, 'UniformOutput', false));

conn.denMeta = cat(1, conn.denMeta, cellMeta);
classConnectome = cat(1, classConnectome, cellClassConn);
...
dendClasses(1).ids = find( ...
  ...
  & conn.denMeta.targetClass ~= 'FullInput' ...
