...
skeletonFolders = {'MBKMB_L4_dendrite_queries_2017_b_nmls'};
...
cell2mat(ff.startNode(find(cellfun(@(x)max([0,ismember(x,y_pre.dendrite1.foundAgglomerates_col{2})]), startAgglo)))')
ff.filenames(find(cellfun(@(x)max([0,ismember(x,y_pre.dendrite1.foundAgglomerates_col{1})]), endAgglo)))'
last=@(x)x{end};

for idx = 1 : 8
    folder = ['/gaba/scratch/kboerg/eval_agglo/postQueryPost/dendrites1/query_' num2str(idx) '_'];
    usedAgglos = cell2mat(eqClassCC(cellfun(@(x)any(ismember(x, y_pre.dendrite1.foundAgglomerates_col{idx})),eqClassCC)));
