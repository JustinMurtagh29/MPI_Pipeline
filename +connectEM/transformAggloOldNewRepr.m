...
assert(arrayfun(@(x)isempty(setdiff(aggloOld{x},aggloNew(x).nodes(:,4))), 1:numel(aggloOld)));
assert(arrayfun(@(x)isempty(setdiff(aggloNew(x).nodes(:,4),aggloOld{x})), 1:numel(aggloOld)));
