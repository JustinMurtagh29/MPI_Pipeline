...
for e=1:len
  ...
  if(~isempty(name) && len==1)
      if ~strcmp(name,'possibleEnds')
        txt=sprintf('%s%s"%s": {\n',txt,repmat(sprintf('\t'),1,level+(len>1)), checkname(name,varargin{:}));
      else
        txt=sprintf('%s%s"%s": [\n%s\t{\n',txt,repmat(sprintf('\t'),1,level+(len>1)), checkname(name,varargin{:}), padding0);
      end
  else
  ...
  if(len>1) txt=sprintf('%s%s}',txt,repmat(sprintf('\t'),1,level+(len>1))); end
  if(len==1) txt=sprintf('%s%s\t}',txt,repmat(sprintf('\t'),1,level+(len>1))); end
  if(e==len) sep=''; end
  if(e<len) txt=sprintf('%s%s',txt,sprintf(',\n')); end
end
if(len>1) txt=sprintf('%s\n%s]',txt,padding0); end
if(len==1 && (strcmp(name,'possibleEnds') || strcmp(name,'resolutions'))) txt=sprintf('%s\n%s]',txt,padding0); end
...
if(length(size(item))>2 || issparse(item) || ~isreal(item) || jsonopt('ArrayToStruct',0,varargin{:}))
    ...
    if(isempty(name))
        ...
        if numel(item)==1 && strcmp(name, 'resolutions')
