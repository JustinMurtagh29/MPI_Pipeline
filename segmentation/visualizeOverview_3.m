...
%                 x(par) = param.totalPathLength/a.eval.merge.sum;
%                 y(par) = param.totalPathLength/a.eval.split.sum;
%                 z(par) = a.eval.general.maxNrObjects;
%             end
%             plot3(x, y, z, [marker{mod(map,13) + 1} autoKLEE_colormap(algo,:)], 'MarkerSize', markerSize(r));
%             hold on;
    ...
    for algo=1:size(paramCell,2)
        ...
        for r=1:length(param.r)
            for par = 1:length(paramCell{algo})
                a = load([param.dataFolder param.outputSubfolder param.affMaps(map).name filesep 'evaluation' num2str(param.r(r)) '-' num2str(algo) '-' num2str(par) '.mat']);
                % markers: 
                plot3(param.totalPathLength/a.eval.merge.sum/1000, param.totalPathLength/a.eval.split.sum/1000, a.eval.general.maxNrObjects, marker{mod(floor(par./length(paramCell{algo}{par}{2})),13) + 1}, 'MarkerEdgeColor', autoKLEE_colormap(par,:), 'MarkerSize', markerSize(r));
                hold on;
