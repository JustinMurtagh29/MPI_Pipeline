...
for i=1:size(paramCell,2)
    for j=1:length(paramCell{i})
        ...
        save([param.dataFolder param.outputSubfolder nameAffMap filesep 'seg' num2str(r) '-' num2str(i) '-' num2str(j) '.mat'], 'segmentation');
        %evaluate
        eval = evaluateSeg(segmentation, param.skel, param.nodeThres);
        save([param.dataFolder param.outputSubfolder nameAffMap filesep 'evaluation' num2str(r) '-' num2str(i) '-' num2str(j) '.mat'], 'eval');
    end
end
