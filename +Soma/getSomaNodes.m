...
m = load(strcat('/gaba/u/mberning/results/pipeline/20170217_ROI/soma/Nuclei/Nucleus',...
    int2str(somaID), '.mat'));
nucleus = m.nucleus;
    
% % NOTE (BS): Why separate file? Delete of possible?
...
if size(nucleusSegIds,1) == 0
    ...
    nodes = center;
    name = strcat('soma', int2str(somaID), '_', num2str(probThreshold), ...
        '_', num2str(sizeThreshold), '_Agglo');
    return;
end
