function skel = writeSkeletonEdges(graph, com, cc, edges, prob, unusedIds);
    ...
    for tr=1:length(cc)
        if ~isempty(cc{tr})
            ...
            theseCoM = com(cc{tr},:);
            idx = ismember(edges,cc{tr});
            idx = idx(:,1) | idx(:,2);
            theseEdges = edges(idx,:);
            theseEdges2 = theseEdges;
            theseProb = prob(idx);
            for no=1:size(theseCoM,1)
                if any(unusedIds == cc{tr}(no))
                    tempIdx = find(any(theseEdges2 == cc{tr}(no),2));
                    cS = 'Probability: ';
                    for i=1:length(tempIdx)
                        tE = theseEdges2(tempIdx(i),:);
                        cS = [cS num2str(tE(tE ~= cc{tr}(no))) '-' num2str(theseProb(tempIdx(i)), '%.2f') ' '];
                    end
