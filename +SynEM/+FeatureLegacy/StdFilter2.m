classdef StdFilter2 < SynEM.Feature.TextureFeature
    %STDFILTER Local standard deviation filter. Alternative implementation
    % to standard filter because matlab internally converts the data to
    % double thus increasing memory usage. Boundary handling is slightly
    % different (symmetric padding from imfilter vs zero pading in convn).
    % PROPERTIES
    ...
    methods
        function obj = StdFilter2(nhood, sigma)
            obj.name = 'StdFilt';
            obj.nhood = nhood;
            if exist('sigma','var')
                obj.sigma = sigma;
