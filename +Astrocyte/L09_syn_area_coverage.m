...
%% find area of synaptic cleft in um2

lut_syn_area = zeros(size(lut_syn));
synIds_de = unique(setdiff(synVolume_de, 0));

for i = 1:length(synIds_de)

    lut_syn_area(syn_idx(i)) = synaptic_area_function();

end

setdiff(lut_syn_area, 0)

% plot area vs coverage of synapses

plot(lut_syn_area(logical(lut_syn~=0)), lut_syn_int(logical(lut_syn~=0)), '*')
