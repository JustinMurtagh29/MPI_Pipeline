...
load /zdata/manuel/code/auxiliary/autoKLEE_colormap.mat;
% Bounding Box (in global coordinates) from KLEE tracing
[kn_tracing, bbox] = findBBoxKLEE(kn_tracing);
% Go to local tracing (THE FAMOUS KLEE X/Y SWITCH IS UNDONE HERE & in changeToLocalTracing)
bbox([1 2],:) = bbox([2 1],:);
kn_tracing = changeToLocalTracing(kn_tracing, bbox(:,1));
% Preallocate stack matrix as unsigned integer
kn_stack = zeros(bbox(:,2)' - bbox(:,1)' + [1 1 1], 'uint8' );
% Create two grids which having the size of the bbox
[kn_gridx, kn_gridy] = ndgrid( 1:bbox(1,2)-bbox(1,1)+1, 1:bbox(2,2)-bbox(2,1)+1 );  
...
for i=1:size( kn_tracing.contourList, 1 )
        ...
        if kn_thisz < 1 || kn_thisz > bbox(3,2)-bbox(3,1)+1
